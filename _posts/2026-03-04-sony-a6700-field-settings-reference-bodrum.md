---
title: "Sony A6700 Field Settings Reference — Bodrum"
date: 2026-03-06
...
1. **AF-S → AF-C** (continuous focus, tracks moving subjects)
2. **Turn on Human Tracking** (camera finds and follows the person)
...
> **Focus trick**
> Stand where you will pose, half-press to focus (green box confirms), then press shutter to start the 10-second timer and get into position. If focus slips, flip the lens switch to MF after the green box locks — this freezes focus so the camera cannot change it. Flip back to AF when done.
{: .prompt-tip }
...
> **Procedure**
> 1. Set up tripod
> 2. Compose the shot without the ND filter
> 3. Focus and lock to MF (the camera may struggle to autofocus through a dark ND filter)
> 4. Attach the ND filter
> 5. Start at 6 seconds shutter, take a test shot
> 6. Too bright — go faster or use a stronger ND. Too dark — go slower
> 7. Check histogram — well spread with no spike hard against either edge
{: .prompt-tip }
...
> **Procedure**
> 1. Set up tripod at eye level — too low and water blocks the view, too high and you see the top of your head
> 2. Frame the entry point and swim path
> 3. Hit record, wait 5 seconds, check playback for exposure and zebras
> 4. If good, hit record again and go swim
> 5. Use phone app to start/stop recording remotely if available
{: .prompt-tip }
...
1. Position playhead where slow-mo starts (just before dive)
2. Press C (razor tool) to cut, same where slow-mo ends. Press V to switch back
3. Right-click the cut section → Speed/Duration → 50%

**Scale-in on impact:**

1. Keyframe Scale in Effect Controls — 100% one second before the dive, 120% at the splash
2. Right-click each keyframe → Temporal Interpolation → first keyframe Ease Out, second keyframe Ease In

**Speed ramp back to normal:**

1. Right-click clip → Show Clip Keyframes → Time Remapping → Speed
2. Ctrl + Click on the rubber band line to add two keyframes near the end of the slow-mo section
3. First keyframe at 50%, second at 100% — the clip ramps smoothly from slow-mo to normal speed

> **Overexposure lesson**
> If a video clip is heavily overexposed, Lumetri Color can pull some back with Highlights -100 and Whites -50, but if the sky and water are flat white the detail is gone. A dark underexposed video clip is more recoverable than a bright overexposed one — same rule as photos.
{: .prompt-warning }
